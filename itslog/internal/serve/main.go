...
func PourGin(apiKeys config.ApiKeys, ch_evt_out chan<- *itslog.Event) *gin.Engine {
	...
	apiV1.GET("/health", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"ok": "go"})
	})
	...
	auth_testV1 := apiV1.Group("/")
	auth_testV1.Use(AuthMiddleWare([]string{config.KEY_KIND_TEST}, apiKeys))
	auth_testV1.PUT("dse/:date/:appID/:eventID", Event("dse", ch_evt_out))
	auth_testV1.PUT("dsev/:date/:appID/:eventID/:value", Event("dsev", ch_evt_out))
